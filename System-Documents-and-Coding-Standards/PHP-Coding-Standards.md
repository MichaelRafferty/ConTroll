...
ConTroll is developed using PHP 8.2 or later.  It currenly does not use PHP classes for its own code, 
but that use is not precluded in future development.
...
- The following arguments are common to each function:
    - `$query` is the text string of the query.
        - Variable substitution is performed with the prepare and ? notation of mysql/mariadb.
    - `$typestr` is the string concatenating the data type of each argument in the `$valueArray`
        - i = integer
        - d = fixed point decimal and floating point
        - s = string
        - b = blob
    - `$valueArray` = array of values for each parameter to be substituted
...
- Note: `sql_safe($string)` and `fetch_safe_assoc($res)` should no longer be used
    - Recode the query and use the Safe versions of the database calls instead.

## HTML:
All HTML output is in Bootstrap 5.
 - the sm column classes should be used.
...
- Color Usage:
 ...
 - `<div class="container-fluid">` should wrap all output
     - multiple `container-fluids` are allowed on a single page
     - `container-fluids` may nest to split the column area
- `<div class="modal" id="ID" tabindex="index" aria-labelledby="label" aria-hidden="true">` style should be used for all modal popups
    - All modals should 
  include a 
    - header
      - contains the title and should have an id for the title string area so Javascript can alter it
    - body
      - contains the main divs
      - a separate button area for actions within the body
    - footer
      - button area for actions on saving, discarding and closing the modal
- a separate div for the message area to be used by `show_message()`

- forms should use `<label>` for 'label for' fields, and all input type fields should have both id and name

### Message output blocks
- The last div in the modal footer, or the container-fluid of the page is:

      <div id='result_message' class='mt-4 p-2'></div>

  - This is the default name for the div, and the one used on the main page. 
  - Each modal should have this as the last div of the footer. 
      - In this case the name should be `xxx_result_div` where
