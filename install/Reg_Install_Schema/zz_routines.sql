...
CREATE ALGORITHM=UNDEFINED 
SQL SECURITY INVOKER
VIEW `memLabel` AS select `m`.`id` AS `id`,`m`.`conid` AS `conid`,`m`.`sort_order` AS `sort_order`,`m`.`memCategory` AS `memCategory`,`m`.`memType` AS `memType`,`m`.`memAge` AS `memAge`,`a`.`shortname` AS `ageShortName`,`m`.`label` AS `shortname`,concat(`m`.`label`,' [',`a`.`label`,']') AS `label`,`m`.`cartDesc` AS `cartDesc`,`m`.`notes` AS `notes`,`m`.`price` AS `price`,`m`.`badgeLabel` AS `badgeLabel`,`m`.`startdate` AS `startdate`,`m`.`enddate` AS `enddate`,`m`.`atcon` AS `atcon`,`m`.`online` AS `online`,`m`.`glNum` AS `glNum`,`m`.`glLabel` AS `glLabel`,`c`.`taxable` AS `taxable`,`c`.`badgeLabel` AS `catBadgeLabel` from ((`memList` `m` join `ageList` `a` on(((`m`.`memAge` = `a`.`ageType`) and (`m`.`conid` = `a`.`conid`)))) join `memCategories` `c` on((`m`.`memCategory` = `c`.`memCategory`))) ;
...
-- Dump completed on 2026-06-11 13:38:24
