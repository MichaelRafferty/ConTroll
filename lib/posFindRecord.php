<?php
// library AJAX Processor: posFindRecord.php
...
SELECT DISTINCT p.id AS perid, TRIM(p.first_name) AS first_name, TRIM(p.middle_name) AS middle_name, 
    ...
    TRIM(p.badge_name) AS badge_name, TRIM(p.badgeNameL2) AS badgeNameL2,
...
SELECT DISTINCT r1.perid, r1.id as regid, m.conid, r1.price, r1.paid, r1.paid AS priorPaid, r1.couponDiscount, r1.coupon,
    ...
    n.reg_notes, n.reg_notes_count, m.memCategory, m.memType, m.memAge, m.shortname, rs.tid as rstid, m.badgeLabel,
...
EOS;
        ...
        if ($usePortal == 1 && getSessionVar("POSMode", 'checkin') == 'checkin') {
            $overlapQ = <<<EOS
