...
//      GOOGLE is a required section, the others are optional
...
// Failed to get user details
    ...
    // amazonAuth - use Oauth2 to retrieve email and name of user from Amazon
    //      redirectURI = which program is making the Oauth call, this is the path for it's return information.
    function amazonAuth($redirectURI = null) {
        // first check for an error in the prior pass
        if (!empty($_GET['error'])) {
            $oauthParams['error'] = htmlspecialchars($_GET['error'], ENT_QUOTES, 'UTF-8');
            return $oauthParams;
        ...
        // so we get back to here, mark that we are doing a amazon authentication session
        setSessionVar('oauth2', 'amazon');
        setSessionVar('oauth2pass', 'startup');
        $provider = new amazon([
...
// Failed to get user details
    ...
    // microsoftAuth - use Oauth2 to retrieve email and name of user from Microsoft Entra ID
    //      redirectURI = which program is making the Oauth call, this is the path for it's return information.
    function microsoftAuth($redirectURI = null) {
        // first check for an error in the prior pass
        if (!empty($_GET['error'])) {
            $oauthParams['error'] = htmlspecialchars($_GET['error'], ENT_QUOTES, 'UTF-8');
            return $oauthParams;
        ...
        // so we get back to here, mark that we are doing a microsoft authentication session
        setSessionVar('oauth2', 'microsoft');
        setSessionVar('oauth2pass', 'startup');
...
// Failed to get user details
    ...
    // facebookAuth - use Oauth2 to retrieve email and name of user from Facebook
    //      redirectURI = which program is making the Oauth call, this is the path for it's return information.
    function facebookAuth($redirectURI = null) {
        // first check for an error in the prior pass
        if (!empty($_GET['error'])) {
            $oauthParams['error'] = htmlspecialchars($_GET['error'], ENT_QUOTES, 'UTF-8');
            return $oauthParams;
