...
$ordinal = substr($prefix, 2);
$ordinal = str_replace('_', '', $ordinal) + 1;
$which = ($type == 'i' ? 'Included Member ' : 'Additional Member ') . "$ordinal: ";

$validated = getUSPSNormalizedAddress($address, $address2, $city, $state, $zip, $which);
...
if (!is_array($validated)) {
    $response['error'] = $which . $validated;
    ajaxSuccess($response);
    exit();
}

if (array_key_exists('error', $validated)) {
    $response['error'] = $validated['error']['code'] . ': ' . $which . $validated['error']['message'];
    ajaxSuccess($response);
    exit();
}
