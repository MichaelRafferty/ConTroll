...
EOS;
    ...
    case "memlist":
        ...
        $addSQL = <<<EOS
INSERT INTO memList(conid,sort_order,memCategory,memType,memAge,label,notes,cartDesc,price,startdate,enddate,atcon,online,glNum,glLabel)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
EOS;
        $addtypes = 'iisssssssssssss';
        $updSQL = <<<EOS
UPDATE memList
SET sort_order = ?,memCategory = ?,memType = ?,memAge = ?,label = ?,notes = ?, cartDesc = ?, price = ?,startdate = ?,enddate = ?,
    atcon = ?,online = ?, glNum = ?, glLabel = ?
WHERE id = ?
EOS;
        $updtypes = 'isssssssssssssi';

        foreach ($data as $row) {
            if (!array_key_exists('notes', $row))
                $row['notes'] = null;
            if (!array_key_exists('cartDesc', $row))
                $row['cartDesc'] = null;
            else if ($row['cartDesc'] != null) {
                $row['cartDesc'] = trim($row['cartDesc']);
                if ($row['cartDesc'] == '')
                    $row['cartDesc'] = null;
            }
            if (!is_numeric($row['id']) || $row['id'] < 0) {
                $paramarray= array($row['conid'],$row['sort_order'],$row['memCategory'],
                    ...
                    $row['startdate'],$row['enddate'],$row['atcon'],$row['online'],$row['glNum'],$row['glLabel']);
                ...
                $paramarray = array($row['sort_order'],$row['memCategory'],
                    $row['memType'],$row['memAge'],$row['shortname'],$row['notes'],$row['cartDesc'],$row['price'],
                    $row['startdate'],$row['enddate'],$row['atcon'],$row['online'],$row['glNum'],$row['glLabel'],$row['id']);
